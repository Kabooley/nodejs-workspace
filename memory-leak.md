...
投稿者によるメモリリーク記録アプリを使って実際にpuppeteerアプリケーションのメモリ使用を記録してみる。

`drawRamOnChart.js`は、root/drawRamChart/というスペースを設けた。

注意：

- 記録する対象のアプリケーションは`node <APPLICATION-NAME> --tagprocess`と、オプションは最後につけること

- 記録するアプリケーションの起動は`node drawRamOnChart.js 'tagprocess`という呼出方をすること

TODO: gitのアップロードを忘れの解消をしたら実際に試してみること。