...
/*********************************************************************
 * Promiseチェーンの一つがPromise.all()を返したらどうなるのか等の検証
 *
 * 検証：
...
// }
// /*******************************************************************
//  * Promiseチェーンの一つがPromise.all()を返すコードの修正方法の模索
//  * 
//  * *****************************************************************/ 
// {
//     // Definition of async functions
...
*/
{
    ...
    Promise.all([
        new Promise((resolve, reject) => {
            sequences[0].then(() => resolve("PROMISE 0 DONE")).catch((e) => reject(e));
        }),
        new Promise((resolve, reject) => {
            sequences[1].then(() => resolve("PROMISE 1 DONE")).catch((e) => reject(e));
        }),
        new Promise((resolve, reject) => {
            sequences[2].then(() => resolve("PROMISE 2 DONE")).catch((e) => reject(e));
        }),
    ])
        .then((r) => {
        console.log("All done.");
